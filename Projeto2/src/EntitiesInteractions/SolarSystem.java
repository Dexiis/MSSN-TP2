...
public class SolarSystem extends PApplet {
    ...
    private final float speedUp = 60 * 60 * 24; // segundos * minutos * horas = 1 dia por calculo
    
    private final float ORBIT_SPACING_PIXELS = 50.0f; // Isto serve para todos os planetas estarem à mesma distância VISUALMENTE
    ...
    private void initializeSolarSystem() {
        ...
        Body sun = new Body(new PVector(0, 0), new PVector(0, 0), SUN_MASS, 
                            (float)(6.957e8 * SUN_RADIUS_SCALE), color(255, 180, 0), 1.0f, 4.0f);
        bodies.add(sun); 
        currentVisualPosition += ORBIT_SPACING_PIXELS;
        
        addPlanet(3.30e23f, 2.44e6f, 0.387f * EARTH_DIST_AU, 4.79e4f, color(150, 150, 150), currentVisualPosition, 0.3f);
        currentVisualPosition += ORBIT_SPACING_PIXELS;
    ...
    private void addPlanet(float mass, float radius, float distance, float speed, int planetColor, float targetVisualDistance, float customRadiusScale) {
        ...
        Body planet = new Body(position, velocity, mass, 
                               (float)(radius * PLANET_RADIUS_SCALE), planetColor, scaleFactor, customRadiusScale);
        bodies.add(planet);
    }
}